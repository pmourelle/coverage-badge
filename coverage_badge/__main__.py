...
def get_total_from_html(htmldir):
    """
    Return the total from index.html.
    """
    with open(os.path.join(htmldir, "index.html"), "r") as f:
        html_content = f.read()
...
def parse_args(argv=None):
    ...
    parser.add_argument('-d', '--htmldir', dest='htmldir',
            help='Get coverage from index.html file (generated by coverage).')
...
def main(argv=None):
    ...
    # Generate badge
    try:
        if args.htmldir and os.path.exists(args.htmldir):
            total = get_total_from_html(args.htmldir)
        else:
            total = get_total()
